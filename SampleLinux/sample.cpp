...
GLfloat light_position2[] = {5., 5., 3., 0.0};
// main program:
int        Distort;
int on1;
int on2;
...
Display( )
{
    ...
    //inside view
    if (WhichView == 1){
        gluLookAt(-0.4, 1.8, -2.9, -0.4, 1.8, -8., 0., 1., 0.);
    }
    else{
     gluLookAt(8., 5., 11.,        0., 0., 0.,     0., 1.,  0.);
    ...
    glCallList(Proj3List1);

    glPushMatrix();
    SetMaterial(1,1,1,1);
    glTranslatef(0, 0, -12 * Time);
    glutSolidSphere(0.25, 100, 100);
    glPopMatrix();

    glPushMatrix();
    glTranslatef(0, 0, -4 * Time);
    SetMaterial(0., 0., 1, 0.5 );
    glShadeModel(GL_FLAT);
    glutSolidTorus(0.275*4, 0.85*4, 8, 15);
    glPopMatrix();


    glPushMatrix();
    glTranslatef(0, 0, -12 * Time);
    SetPointLight(GL_LIGHT1, -1, -1, -1, 1, 1, 1);
    glPopMatrix();
...
void SetPointLight(int i, float x, float y, float z, float r, float g, float b){
    glLightfv(i, GL_POSITION, Array3(x, y, z));
    glLightfv(i, GL_AMBIENT, Array3(0., 0., 0.));
    glLightfv(i, GL_DIFFUSE, Array3(r, g, b));
    glLightfv(i, GL_SPECULAR, Array3(r,g,b));
    glLightf(i, GL_CONSTANT_ATTENUATION, 1.);
    glLightf(i, GL_LINEAR_ATTENUATION, 0.);
    glLightf(i, GL_QUADRATIC_ATTENUATION, 0.);
}

void SetSpotLight(int i, float x, float y, float z, float xdir, float ydir, float zdir, float r, float g, float b){

    glLightfv(i, GL_POSITION, Array3(x, y, z));
    glLightfv(i, GL_SPOT_DIRECTION, Array3(xdir, ydir, zdir));
    glLightf(i, GL_SPOT_EXPONENT, 3.);
    glLightf(i, GL_SPOT_CUTOFF, 60);
    glLightfv(i, GL_AMBIENT, Array3(0., 0., 0.));
    glLightfv(i, GL_DIFFUSE, Array3(r, g, b));
    glLightfv(i, GL_SPECULAR, Array3(r,g,b));
    glLightf(i, GL_CONSTANT_ATTENUATION, 1.);
...
InitMenus( )
{
    ...
    glutAddSubMenu( "Texture Options", texmenu);
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
		...
		for( int ilng = 0; ilng < NumLngs; ilng++ )
		{
			...
			if( Distort==1 )
			{
             //   p->s = 0;
           //     p->t = 0;
				p->s = Time * ( lng + M_PI    ) / ( 2.*M_PI );
		        p->t = Time * Time *  ( lat + M_PI/2. ) / M_PI;
	//			p->s = Time * p->x;// p->t;
	//			p->t = Time * p->y; //p->s;
			}
			else
...
InitLists( )
{
    ...
    Proj3List1 = glGenLists(1);
    glNewList(Proj3List1, GL_COMPILE);

	glEnable( GL_NORMALIZE );
    glEnable(GL_LIGHTING);
    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, MulArray3(.2, White));
    glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_TRUE);
    SetPointLight(GL_LIGHT0, 0, 0, 4.5, 1, 1, 1);
    SetSpotLight(GL_LIGHT2, 5, 5, 3, 6, 6, 2, 1, 0.3, 0);

    glPushMatrix();
    SetMaterial(1, 0.5, 0, 8);
    glShadeModel(GL_SMOOTH);
    glutSolidTorus(0.275, 0.85, 8, 15);
    glPopMatrix();

    glPushMatrix();
    SetMaterial(1., 0.5, 0, 0);
    glShadeModel(GL_SMOOTH);
    glutSolidTorus(0.275*9, 0.85*9, 25, 50);
    glPopMatrix();
            ...
            for (j = 0; j<6; j++){
                for (i = 0; i<=360; i++){
                    ...
                    if (j==2){
                        ...
                        glColor3f(1., 0.98, 0.24);
                    }
                    if (j==3){
                        offset = 1.25;
                        glColor3f(0.52, 1., 0.04);
                    }
                    if (j==4){
                        offset = 0.95;
                        glColor3f(0., 0., 1.);
                    ...
                    glVertex3f(sin(i)/(j+1) * offset, cos(i)/(j+1) * offset, 0.25*j);
